...
@main.route('/search-products', methods=['GET'])
def search_products():
    """
    Buscar productos por nombre o por categoría
    ---
    tags:
      - Productos
    parameters:
    ...
    responses:
      200:
        description: Lista de productos filtrados
      404:
        description: Ningún producto encontrado
    """
    try:
        nombre = request.args.get('nombre')      
        categoria_id = request.args.get('categoria_id')  

        productos = ProductoModel.buscar(nombre, categoria_id)
        if not productos:  # Lista vacía
            return jsonify({"message": "Ningún producto encontrado"}), 404
